...
test('ignora ora di stampa, intestazioni dei giorni e testo fuori griglia',()=>{const p=page(146,'RAIOLA ANGELO','SCIENZE ALIMENTARI');p.items.push(item('11/09/2026 08:31 - Pagina 146',430,30,140),item('lunedì 14/09',75,60,65),item('Orario in vigore dal 14 settembre 2026',380,385,180));const result=parseIndexEducationPages([p],{firstName:'Michelangelo',lastName:'Raiola',fullName:'Michelangelo Raiola'});assert.ok(result.lessons.length>0);assert.ok(result.lessons.every(l=>!l.subject.match(/LUNED|MARTED|MERCOLED|GIOVED|VENERD|SABAT/i)));assert.ok(result.lessons.every(l=>l.time!=='08:31 – 08:00'))});
