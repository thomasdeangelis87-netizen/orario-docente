...
function detectTeacherHeader(page){const lines=groupLines(page.items),days=[];for(const line of lines){if(DAY_NAMES.some(d=>norm(line.text).replace(/ /g,'').startsWith(d)))days.push(line.y)}if(!days.length)return null;const dayY=Math.min(...days),candidates=lines.filter(line=>line.y>Math.max(0,dayY-32)&&line.y<dayY-3).map(line=>({line,text:line.text.trim(),name:norm(line.text)})).filter(c=>{const ts=tokens(c.name);return ts.length>=2&&!/\d/.test(c.name)&&c.line.items.some(i=>i.x>page.width*.18&&i.x<page.width*.82)}).sort((a,b)=>b.line.y-a.line.y);if(!candidates.length)return null;const c=candidates[0];return{name:c.text.replace(/\s+/g,' ').trim(),normalized:c.name,pageNumber:page.number,y:c.line.y}}
