(function(root){
 ...
 function normalized(entries){
  return (entries||[]).map(e=>({day:Number(e.day),period:Number(e.period),activity:clean(e.activity),subject:clean(e.subject),teacher:clean(e.teacher),time:clean(e.time),room:clean(e.room)}))
 ...
 function changes(before,after){
  ...
  const key=e=>`${e.day}-${e.period}:${e.teacher}:${e.activity}:${e.subject}:${e.time}:${e.room}`;
